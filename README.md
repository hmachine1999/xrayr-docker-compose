...
```
docker pull hiepvq/xrayr:latest && docker run --restart=always --name xrayr -d -v ${PATH_TO_CONFIG}/config.yml:/etc/XrayR/config.yml --network=host hiepvq/xrayr:latest
```
# Docker compose 安装
...
2. `git clone https://github.com/hiepvq/xrayr-docker-compose XrayR-release`
3. `cd XrayR-release`
4. 编辑config。
